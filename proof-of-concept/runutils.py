...
def get_cuda_state(obj: object) -> bool:
    """
...
def is_cuda_consistent(*args: object) -> bool:
    """
...
def make_cuda_consistent(refobj: object, *args: object) -> tuple:
    """
    ...
    if ref_cuda_state is None:
        raise ValueError(
            "cannot determine the cuda state of `refobj` ({})".format(refobj)
        )
    ...
    for v in args:
        ...
        if cuda_state != ref_cuda_state:
            ...
            if isinstance(v, nn.Module):
                ...
                v = move_to_device(v)  # noqa: PLW2901
        result_args.append(v)
    return tuple(result_args)


def predict(net: nn.Module, inputs: torch.Tensor) -> torch.LongTensor:
    """
    ...
    return torch.max(outputs_var.data, dim=1)[1]
