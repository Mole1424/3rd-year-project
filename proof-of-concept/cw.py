...
Implemented by Kaiwen, available at https://github.com/kkew3/pytorch-cw2
...
def _var2numpy(var: Variable) -> np.ndarray:
    """
...
def atanh(x: float, eps: float = 1e-6) -> float:
    """
...
def to_tanh_space(
    x: Union[Variable, torch.FloatTensor], box: Tuple[float, float]
) -> Union[Variable, torch.FloatTensor]:
    """
...
class L2Adversary(object):
    ...
    def __init__(  # noqa: PLR0913
        ...
        if c_range[0] >= c_range[1]:
            raise ValueError(
                "c_range lower bound ({}) is expected to be less "
                "than c_range upper bound ({})".format(*c_range)
            )
        if len(box) != 2:  # noqa: PLR2004
            raise TypeError(
                "box ({}) should be of form "
        ...
        self.repeat = self.binary_search_steps >= 10  # noqa: PLR2004
    ...
    def __call__(  # noqa: PLR0912, PLR0915
        ...
        assert len(inputs.size()) == 4  # noqa: PLR2004
        ...
        for sstep in range(self.binary_search_steps):
            ...
            for i in range(batch_size):
                tlabel = targets_np[i]
                assert best_l2_ppred[i] == -1 or self._attack_successful(
                    best_l2_ppred[i], tlabel
                )
                assert o_best_l2_ppred[i] == -1 or self._attack_successful(
                    o_best_l2_ppred[i], tlabel
                )
                if best_l2_ppred[i] != -1:
                    # successful; attempt to lower `scale_const` by halving it
                    upper_bounds_np[i] = min(scale_consts_np[i], upper_bounds_np[i])
                    ...
                    lower_bounds_np[i] = max(scale_consts_np[i], lower_bounds_np[i])
                    if upper_bounds_np[i] < self.c_range[1] * 0.1:
                        scale_consts_np[i] = (
                            lower_bounds_np[i] + upper_bounds_np[i]
                        ) / 2
    ...
    def _optimize(  # noqa: PLR0913
        self,
        model: nn.module,
        optimizer: optim.Adam,
        inputs_tanh_var: Variable,
        pert_tanh_var: Variable,
        targets_oh_var: Variable,
        c_var: Variable,
        ...
        maxother_activ_var = torch.max(
            ((1 - targets_oh_var) * pert_outputs_var - targets_oh_var * inf), 1
        )[0]
        ...
        if self.targeted:
            ...
            # noinspection PyArgumentList
            f_var = torch.clamp(
                maxother_activ_var - target_activ_var + self.confidence, min=0.0
            )
        else:
            ...
            # noinspection PyArgumentList
            f_var = torch.clamp(
                target_activ_var - maxother_activ_var + self.confidence, min=0.0
            )
    ...
    def _attack_successful(self, prediction: int, target: int) -> bool:
        """
    ...
    def _to_tanh_space(self, x: torch.FloatTensor) -> torch.FloatTensor:
        """
    ...
    def _from_tanh_space(self, x: torch.FloatTensor) -> torch.FloatTensor:
        """
